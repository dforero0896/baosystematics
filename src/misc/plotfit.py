...
outdir = sys.argv[3]
title = sys.argv[4]
name=os.path.splitext(os.path.basename(mean_file))[0]+'_'+title.lower().replace(' ','')
...
plot_bao(mean_data[:,0], mean_data[:,1], label = 'Mean', c= 'r', lw = 2)
plot_bao(bestfit_data[:,0], bestfit_data[:,2], label = 'Best fit', c = 'b', lw = 3, ls = '--')
