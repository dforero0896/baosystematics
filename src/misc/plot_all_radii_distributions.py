...
if __name__ == '__main__':
    ...
    store_means=np.empty((len(data_bins), 100))
    ...
    fhist1 = interpolate.interp1d(Rbins, store_means[0])
    fhist2 = interpolate.interp1d(Rbins, store_means[1])
    Rstar = optimize.fsolve(lambda x: fhist1(x) - fhist2(x), x0 = 12.75)
    ax.plot(Rstar, fhist1(Rstar), 'ro', label = r'$R^* = %.3f$ $h^{-1}$ Mpc'%Rstar)
