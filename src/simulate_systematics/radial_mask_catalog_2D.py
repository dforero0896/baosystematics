...
def radial_mask_catalog_2D(fn, odir, sigma=0.235, center=0.5,\
    ...
    rr_file_mask = os.path.join(os.path.dirname(random_mask), 'RR_2d'+os.path.basename(random_mask))
    random_shuf=f"{obases[0]}/void_ran/void_ran.dat"
    rr_file_shuf = os.path.join(os.path.dirname(random_shuf), 'RR_2d'+os.path.basename(random_shuf))
    ...
    for i , on in enumerate(onames):
        ...
        tpcf_gal_dirs = [obases[i]+f"/tpcf2d_gal_{cat_type}_nowt", \
		obases[i]+f"/tpcf2d_gal_{cat_type}"]
        tpcf_void_dirs = [obases[i]+\
			f"/tpcf2d_void_{cat_type}_nowt_R-{rmin_fid}-{rmax_fid}", \
			obases[i]+\
			f"/tpcf2d_void_{cat_type}_R-{rmin_fid}-{rmax_fid}"]
        tpcf_xgv_dirs = [obases[i]+\
			f"/tpcf2d_xgv_{cat_type}_vnw_gnw_R-{rmin_fid}-{rmax_fid}", \
			obases[i]+\
			f"/tpcf2d_xgv_{cat_type}_vnw_gw_R-{rmin_fid}-{rmax_fid}", \
			obases[i]+\
			f"/tpcf2d_xgv_{cat_type}_vw_gnw_R-{rmin_fid}-{rmax_fid}", \
			obases[i]+\
			f"/tpcf2d_xgv_{cat_type}_vw_gw_R-{rmin_fid}-{rmax_fid}"]
        for weight in [0,1]:
            ...
            if not os.path.exists(rr_file_mask) and isfirst:
                count_mode=7
                ncores = NCORES
                fcfc_gal_command = f"srun -n 1 -c {ncores} {RUN_FCFC_2D} --conf={conf_file_gal} --data={on} --rand={random_mask} --count-mode={count_mode} --dd={dd_file_gal} --dr={dr_file_gal} --rr={rr_file_mask} --output={out_file_gal} --data-wt-col={data_wt_col} --cf-mode={cf_mode} --moment=2 --bin-dim=1\n"
            else:
                ncores=NCORES
                count_mode=3
                fcfc_gal_command = f"srun -n 1 -c {ncores} {RUN_FCFC_2D} --conf={conf_file_gal} --data={on} --rand={random_mask} --count-mode={count_mode} --dd={dd_file_gal} --dr={dr_file_gal} --rr={rr_file_mask} --output={out_file_gal} --data-wt-col={data_wt_col} --cf-mode={cf_mode} --moment=2 --bin-dim=1\n"
		...
		"DR_"+os.path.basename(oname_void[weight]).replace('.dat',\
		 ...
		 "TwoPCF_"+os.path.basename(oname_void[weight]).replace('.dat',\
            ...
            if  not os.path.exists(rr_file_shuf) and isfirst:
                count_mode=7
                ncores = NCORES
                fcfc_void_command = f"srun -n 1 -c {ncores} {RUN_FCFC_2D} --conf={conf_file_void} --data={oname_void[weight]} --rand={random_shuf} --count-mode={count_mode} --dd={dd_file_void} --dr={dr_file_void} --rr={rr_file_shuf} --output={out_file_void} --data-wt-col={data_wt_col} --data-aux-col={void_aux_col} --data-aux-min={rmin} --data-aux-max={rmax} --rand-aux-col={void_aux_col} --rand-aux-min={rmin} --rand-aux-max={rmax} --rand-select=23 --cf-mode={cf_mode}\n"
            else:
                ncores=NCORES
                count_mode=3
                fcfc_void_command = f"srun -n 1 -c {ncores} {RUN_FCFC_2D} --conf={conf_file_void} --data={oname_void[weight]} --rand={random_shuf} --count-mode={count_mode} --dd={dd_file_void} --dr={dr_file_void} --rr={rr_file_shuf} --output={out_file_void} --data-wt-col={data_wt_col} --data-aux-col={void_aux_col} --data-aux-min={rmin} --data-aux-max={rmax} --rand-aux-col={void_aux_col} --rand-aux-min={rmin} --rand-aux-max={rmax} --rand-select=23 --cf-mode={cf_mode}\n"
			...
			"TwoPCF_"+os.path.basename(on).replace('.dat', \
                ...
                if not os.path.exists(dgdv_file):
                    count_mode=2
                    ncores = NCORES
                    fcfc_cross_command = f"srun -n 1 -c {ncores} {RUN_FCFC_2D} --conf={conf_file_void} --rand={on} --data={oname_void[weight]} --count-mode={count_mode} --dr={dgdv_file} --output={out_file_xgv} --data-wt-col={data_wt_cols_void[weight]} --rand-wt-col={data_wt_cols_gal[weight_gal]} --data-aux-col={void_aux_col} --data-aux-min={rmin} --data-aux-max={rmax} --cf-mode=0\n"
                else:
                    ncores=1
...
if __name__ == '__main__':
    ...
    joblist = open(f"joblists/jobs_{space}_radialgauss_{iproc}_box{box}_2D.sh", 'w')

    for i, f in enumerate(filenames_split[iproc]):
        odir = os.path.abspath(os.path.dirname(f)+'/../..')
        command=radial_mask_catalog_2D(f, odir, N_grid = NGRID, rmin = RMIN, rmax = RMAX, \
