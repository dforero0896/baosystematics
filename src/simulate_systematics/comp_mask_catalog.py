...
def comp_mask_catalog(fn, odir, sigma_noise=0.2, function=parabola, cmin=0.8, names = ['x', 'y', 'z'], N_grid=2500, noise_sampler=noise_sampler, rmin=16, rmax=50, cat_type='mock'):
    ...
    oname_gal_noise = os.path.join(odir, f"noise/{function.__name__}_{cmin}/{cat_type}s_gal_xyz/{oname_gal}")
    oname_gal_smooth = os.path.join(odir, f"smooth/{function.__name__}_{cmin}/{cat_type}s_gal_xyz/{oname_gal.replace('sigma'+str(sigma_noise), 'sigma0.0')}")
    onames = [oname_gal_smooth, oname_gal_noise]
    obases = [os.path.join(odir, s,f"{function.__name__}_{cmin}") for s in ['smooth', 'noise']]
    ...
    for i , on in enumerate(onames):
        ...
        oname_void = os.path.join(obases[i], f"{cat_type}s_void_xyz", os.path.basename(on).replace('.dat', f".VOID.dat"))
        ...
        tpcf_gal_dirs = [obases[i]+f"/tpcf_gal_{cat_type}_nowt", obases[i]+f"/tpcf_gal_{cat_type}"]
        tpcf_void_dirs = [obases[i]+f"/tpcf_void_{cat_type}_nowt_R-{rmin}-{rmax}", obases[i]+f"/tpcf_void_{cat_type}_R-{rmin}-{rmax}"]
        tpcf_xgv_dirs = [obases[i]+f"/tpcf_xgv_{cat_type}_vnw_gnw_R-{rmin}-{rmax}", obases[i]+f"/tpcf_xgv_{cat_type}_vnw_gw_R-{rmin}-{rmax}", obases[i]+f"/tpcf_xgv_{cat_type}_vw_gnw_R-{rmin}-{rmax}", obases[i]+f"/tpcf_xgv_{cat_type}_vw_gw_R-{rmin}-{rmax}"]
        ...
        for weight in [0,1]:
            ...
            if weight==0: #Remove this condition to implement void weights
                ...
                dd_file_void = os.path.join(tpcf_void_dirs[weight],"DD_files","DD_"+os.path.basename(oname_void).replace('.dat', f".R-{rmin}-{rmax}.dat"))
                ...
                for weight_gal in [0,1]:
                    ...
                    dgdv_file = os.path.join(tpcf_xgv_dirs[id_], "DD_files", "DD_"+os.path.basename(on).replace('.dat', f".CROSS.R_{rmin}-{rmax}.dat"))
                    out_file_xgv = os.path.join(tpcf_xgv_dirs[id_], "TwoPCF_"+os.path.basename(on).replace('.dat', f".CROSS.R_{rmin}-{rmax}.dat"))
                    if not os.path.exists(dgdv_file):
                        count_mode=2
                        ncores = NCORES
                        fcfc_cross_command = f"srun -n 1 -c {ncores} {RUN_FCFC} --conf={conf_file_void} --rand={on} --data={oname_void} --count-mode={count_mode} --dr={dgdv_file} --output={out_file_xgv} --data-wt-col={data_wt_cols_void[weight]} --rand-wt-col={data_wt_cols_gal[weight_gal]} --data-aux-col=4 --data-aux-min={rmin} --data-aux-max={rmax} --cf-mode=0 && {WORKDIR}/bin/2pcf.py {dgdv_file} {out_file_xgv}\n"
                    else:
                        ncores=1
...
if __name__ == '__main__':
    ...
    for f in filenames_split[iproc]:
        ...
        command=comp_mask_catalog(f, odir, noise_sampler=noise_sampler, function = FUNCTION, cmin = cmin_map, N_grid = 2500, rmin = 16, rmax = 50, sigma_noise=0.2)
