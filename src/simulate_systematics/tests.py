...
for i, function in enumerate(functions):
  ...
  if not os.path.exists(masked_dat_fn):
    ...
    masked_dat = mask_with_function(data, lambda y, x: function(y, x, N_grid, dat_comp_mins[i]), noise=DATA_NOISE, box_size=2500, N_grid=N_grid, sigma_noise=sigma_noise, cmin=cmin_cut)
  ...
  if not os.path.exists(masked_ran_fn):
    ...
    masked_ran = mask_with_function(ran, lambda y, x: function(y, x, N_grid, ran_comp_mins[i]), noise=False, box_size=2500, N_grid=N_grid, sigma_noise=sigma_noise, cmin=cmin_cut)
  ...
  for weight in [0,1]:
    ...
    if weight == 0 : 
      ...
      rand_wt_col=4
      data_wt_col=4
    bash_script.write('srun -n 1 -c 1 %s --conf=%s --data=%s --rand=%s --count-mode=%s --dd=%s --dr=%s --rr=%s --output=%s --rand-wt-col=%s --data-wt-col=%s\n'%(RUN, conf_file, masked_dat_fn, masked_ran_fn, count_mode, dd_file, dr_file, rr_file, out_file, rand_wt_col, data_wt_col))
bash_script.close()
