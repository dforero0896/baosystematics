...
data_aux_min = 16
data_aux_max = 50
mask_dat_fn_list = [f for f in os.listdir(COMP) if f.startswith(os.path.basename(DATA).replace('.dat', ''))]
# Setup fcfc run
...
for mask_dat_base in mask_dat_fn_list:
  ...
  for weight in [0]:
    ...
    if weight == 0 : 
      ...
      rand_wt_col=5
      data_wt_col=5
    bash_script.write('srun -n 1 -c 16 %s --conf=%s --data=%s --rand=%s --count-mode=%s --dd=%s --dr=%s --rr=%s --output=%s --rand-wt-col=%s --data-wt-col=%s --data-aux-col=%s --data-aux-min=%s --data-aux-max=%s\n'%(RUN_FCFC, conf_file, masked_dat_fn, masked_ran_fn, count_mode, dd_file, dr_file, rr_file, out_file, rand_wt_col, data_wt_col, data_aux_col, data_aux_min, data_aux_max))
bash_script.close()
