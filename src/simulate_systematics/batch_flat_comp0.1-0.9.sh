...
#  echo "sbatch -p p4 -n 16 -c 1 --mem-per-cpu=8G -J rc${comp} --wrap='srun -n 32 -c 1 --mem-per-cpu=8G --mpi=pmi2 ./comp_mask_catalog.py /home/epfl/dforero/scratch/projects/baosystematics/patchy_results/box${BOX}/${SPACE}/nosyst/mocks_gal_xyz ${comp}'"
  echo "sbatch -p p4 -n ${NCORES} -c 1 --mem-per-cpu=8G -J ${comp} --wrap='srun -n ${NCORES} -c 1 --mem-per-cpu=8G --mpi=pmi2 ./comp_mask_catalog.py /home/epfl/dforero/scratch/projects/baosystematics/patchy_results/box${BOX}/${SPACE}/nosyst/mocks_gal_xyz -c ${comp} -b ${BOX} -s ${SPACE} -r ${scaled_rmin} -nu 1/3'"
done
done
