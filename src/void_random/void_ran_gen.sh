...
if [[ $# -ne 4 ]]; then
	...
	echo USAGE: $0 RESULTS_DIR R_MIN R_MAX OVERWRITE
	exit 1
fi
...
awk -v rmin="$r_min" -v rmax="$r_max" -F"\t" '($4*1) > rmin && ($4*1) < rmax' $results_dir/void_ran/BIG_RAN_VOID_NGC.ascii | shuf -n 2700000 > $results_dir/void_ran/EZ_ELG_RDZ_void_ran_NGC_R-$r_min-$r_max.ascii
...
awk -v rmin="$r_min" -v rmax="$r_max" -F"\t" '($4*1) > rmin && ($4*1) < rmax' $results_dir/void_ran/BIG_RAN_VOID_SGC.ascii | shuf -n 2700000 > $results_dir/void_ran/EZ_ELG_RDZ_void_ran_SGC_R-$r_min-$r_max.ascii
