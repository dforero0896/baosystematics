...
if __name__ == '__main__':
    ...
    print(f"# Serror\tbias\tstderr\tconf_int_low\tconf_int_high\tsamples_dir\tn_samples")
    for ilist_fn in ilists_fn:
        ...
        Serr, bias, stderr, conf_interval = jackknife_stats(signal_arr, np.std, 0.95)
        print(f"{Serr}\t{bias}\t{stderr}\t{conf_interval[0]}\t{conf_interval[1]}\t{os.path.dirname(ilist[0])}\t{len(signal_arr)}")
