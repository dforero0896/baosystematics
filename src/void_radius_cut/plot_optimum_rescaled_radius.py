...
if __name__ == '__main__':
    ...
    for box in boxes:
        ...
        line, = ax.plot(1e4 * ngals, Rvals*ngals**(-c[1]), marker=(5, 1, 180), lw=0, c = 'k')
        print(np.mean(Rvals*ngals**(-c[1])))
        ax.axhline(np.exp(c[0])/10, c='k', ls='--', label=r'$\bar{n}_{\mathrm{gal}}^{%0.2f}~R^* = %+.2f$ (Mpc/$h$)$^{1/4}$'%(-c[1],np.exp(c[0])/10))
