...
\begin{abstract}
Baryon Acoustic Oscillations (BAO) are a signal imprinted in the 3-dimensional distribution of various tracers in the Universe. It provides a standard scale that can be measured at different redshifts. In the present work, we aim to evaluate the impact of systematical effects in the void BAO signal (as obtained from the discrete distribution of Emission Line Galaxies (ELG) using a Delaunay Triangulation algorithm) as opposed to the galaxy BAO. To do this we compute the optimal radius cut to separate voids-in-voids from voids-in-clouds and find it to be $R_c = \SI{15.5}{\h^{-1}\mega\parsec}$. We see that the dilation parameter measurements hint towards a difference in the impact of systematics on voids vs. galaxies. Nonetheless the error is still relatively high to draw definitive conclusions. We measure $\alpha_{\mathrm{all}} - \alpha_{\mathrm{none}} = \num{0.00189\pm0.00338}$ for galaxies and $\alpha_{\mathrm{all}} - \alpha_{\mathrm{none}} = \num{-0.00107\pm0.00338}$ for voids. Such small differences suppose this analysis should be done with other tracers with better SNR such as LRGs.
\end{abstract}
